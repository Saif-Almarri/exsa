...
if (process.argv.includes('--layers')) {
  ...
  for (const f of layerScoped) {
    ...
    if (!/^@layer\s+exsa\.(components|layouts|themes|skins|fluid)\s*\{/.test(content)) {
      fail(`${f}: not wrapped in block form @layer exsa.{components|layouts|themes|skins|fluid} { … }`);
    }
...
if (process.argv.includes('--baseline')) {
  ...
  console.log(`components: ${cssFiles2.filter((f) => f.startsWith('dist/components/')).length}, themes: ${cssFiles2.filter((f) => f.startsWith('dist/themes/')).length}, skins: ${cssFiles2.filter((f) => f.startsWith('dist/skins/')).length}, layouts: ${cssFiles2.filter((f) => f.startsWith('dist/layouts/')).length}`);
...
/* ---------- rule 18: skin token-only contract ----------
   A skin is a pure material token layer: every rule targets :root only,
   no element selectors, no !important (rule 3), and no backdrop-filter
   property applied to elements — the value belongs inside --surface-backdrop,
   where components consume it. Applying the property directly in a skin
   would risk the renderer-crash path over animated surfaces. */
for (const f of cssFiles2.filter((f) => f.startsWith('dist/skins/') && f.endsWith('.css'))) {
  const src = readFileSync(join(root, f), 'utf8');
  const body = src.replace(/^@layer\s+exsa\.skins\s*\{/, '').replace(/\}\s*$/, '');
  const clean = body.replace(/\/\*[\s\S]*?\*\//g, '');
  for (const m of clean.matchAll(/([^{}]+)\{([^{}]*)\}/g)) {
    const selector = m[1].trim();
    if (!/^:root(\[[^\]]*\])?$/.test(selector)) fail(`${f}: skin rules must target :root only (got "${selector}")`);
    if (/^\s*backdrop-filter\s*:/m.test(m[2])) fail(`${f}: skins must not apply backdrop-filter to elements — put the value in --surface-backdrop instead`);
    if (/--surface-border-control\s*:\s*(none|transparent)/.test(m[2])) fail(`${f}: --surface-border-control must stay visible (no none/transparent) — inputs need a boundary (a11y)`);
  }
}
