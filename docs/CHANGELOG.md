...
- **`store.css` dissolved into the paper model (breaking)** — the store is now a recipe, not a layout file: `general.css` gains the `.layout__announce` piece (fixed full-bleed strip — body padding, the topbar's position, and sticky offsets all follow it automatically via `:has()`), and `.shop__grid` / `--compact` moved into `components/product-card.css` (the cards' own file). The `shop--*` body classes are gone; the Layouts page documents the store recipe (announce + topbar + sticky filter aside + product grid). The manifest layout entry, QA fixtures, and probe rows were removed.

- **`general.css` is now the paper model (breaking)** — the body-class API (`layout--has-topbar`, `layout--aside-left/right`, `layout--centered`, `layout--sticky-footer`, `layout--aside-sticky`, the width toggles) is replaced by an element-driven API: `.layout__page` (the sheet — width/look/height via `--layout-page-*`), `.layout__grid` (the pieces), and optional pieces `.layout__header`, `.layout__aside-start/--end` (logical, RTL-safe), `.layout__content`, `.layout__footer`, plus `.layout__topbar`, `.layout__hero`, `.layout__grid--centered`, `.layout__page--auto`, `.layout__page--frame`, and `.layout__aside-*--sticky` modifiers. Stacking keys on the sheet's own width (query container, media fallback), and `.sidebar--collapsed` on a side piece shrinks its track to icon width. The site pages, docs, cheatsheet, Layouts page, manifest contracts, and QA fixtures all migrated; `new-general.css` (the proof) is deleted.

- **Starter templates moved out of the framework** — `dist/templates/` (fullpage + onepage) leaves EXSA for a dedicated templates project. Removed everywhere: the manifest token catalog, the debug/validate tooling's template roots, the Layouts page sections and nav, the cheatsheet table, the docs/README structure, the README quick start, and the two QA layout fixtures.

### ✨ Added

- **Sidebar rail opens a tray** — in any icon-only rail (`.sidebar--collapsed`, `.sidebar--fluid` at rail width, or the new auto rail) clicking a sub-menu parent now opens a floating tray docked beside the item — the parent's label as the tray title plus the sub-menu items — instead of doing nothing while labels are hidden. `sidebar.js` portals the tray to `<body>` (an in-rail flyout would be clipped by the sidebar's own scroll), mirrors `.sidebar--dark` onto it, docks it beside the clicked item (RTL-aware, viewport-clamped), and closes it on outside click, Escape, scroll, or resize. Sub-menu triggers also gain real `aria-expanded` sync (matching the component's documented ARIA contract) and Space-key activation.

- **Sidebar auto rail on small screens** — at ≤767px a standalone `.sidebar` switches itself to the icon-only rail (same geometry as `.sidebar--collapsed`, width from `--sidebar-collapsed-w`) so a full-width nav column never dominates a small page; sub-menus open in the tray. Layout-shell pieces (`.layout__aside-start` / `.layout__aside-end`) are exempt — their shells already stack them full-width on small screens — as are the developer-chosen `--collapsed` and `--fluid` states.
...
- **Sidebar gains a fluid icon-only variant** — `.sidebar--fluid` is the second collapse design, the developer's choice alongside manual `.sidebar--collapsed`: `container-type: inline-size` + a `@container sidebar (max-width: 120px)` query hide labels, arrows, sub-menus, and badges and center the links whenever the sidebar's *own* width can't fit text. Pair it with a fluid width (e.g. `clamp(56px, 100%, 240px)`) and the switch is automatic at any viewport — no media queries, no JS. Fixed-width hosts (like `layout--aside-left`'s 260px) keep labels, by design.

- **One grid system** — the 8-unit `app-grid.css` experiment was dropped before release: two parallel grid vocabularies (utilities `grid-cols-*` vs `.span-*`) would have doubled docs and drifted apart. The utilities tier is the single grid system, extended with `.grid-cols-7` and `.grid-cols-8`.

- **Layouts own zones only — component parts moved out of dashboard/store** — same purity rule as `general.css`. `dash--sidebar-dark` styling became `.sidebar--dark` in components/sidebar.css (add it to the sidebar element), `.dash__sidebar-head` merged into the component's `.sidebar__head` (which gained flex + gap), and the nav's scrollbar cosmetics moved to `.sidebar__nav`. Store: `.shop__cart-count` is now `badge badge--count badge--solid` (new `.badge--solid` in components/badge.css), `.shop__search` became `.topbar__search` in components/topbar.css (shrinks in its own ≤767px block), and the filter widget (`.shop__aside-title`, `.shop__aside-group`) moved to components/product-card.css. Layout files arrange zones and scrolling; colors, type, and chrome live in components. (Breaking: the removed class names above.)
...
- **The app frame ships as a focus layer** — `.layout__page--app` moved out of `general.css` into `layouts/dashboard.css`: the base stays complete for websites, stores, and blogs, and dashboards link both files (`general.css` + `dashboard.css`, base + increment — one dependency direction, no duplication). The dashboard recipe docs and cheatsheet now show the two-link setup.

- **`general.css` stacking is now shell-relative** — the ≤860px stack was a viewport media query, so a shell embedded in a smaller space than the window (app pane, split view, iframe) mis-fired. The shell is now a flex-wrap row + query container (`exsa-shell`): pieces wrap to full rows keyed to the shell's own width, with the same rules mirrored in a viewport media query for engines without container queries. The both-asides 3-column grid became flex (identical geometry, intrinsic wrapping — the grid was the last viewport-bound construct), and the topbar/footer width-toggle resets stay viewport-keyed by design (fixed chrome spans the viewport). Desktop geometry and the ≤860px threshold are unchanged.

- **Layout reservations are now element-evidenced** — `general.css` reserved space from body classes alone: declaring both asides kept the third grid track even when the right `<aside>` was absent (260px dead track at 1280px), and `body.has-topbar` padded the page with no topbar element in the DOM. The both-asides grid (`general.css`) and the topbar reservation (`topbar.css`) now carry `:has()` element evidence — the same mechanism as the core classless detection — so a declared-but-absent zone consumes nothing and the content track absorbs the space: "a region consumes space only when it exists" is now literally true. The ≤860px reset mirrors the guards to keep the cascade balanced (fixing a real regression in the process: the desktop aside-width rule's `:not(.sidebar--collapsed)` out-specified the mobile 100% reset, leaving the sidebar 220px instead of full-width on phones), and pre-`:has()` engines keep the class-based reservation via `@supports not selector(:has(*))`.

- **Spacing is skin-responsive across all 68 components again** — the 11 components shipped with the skin system (`activity`, `calendar`, `command-palette`, `empty-state`, `kanban`, `kpi`, `notifications`, `tags-input`, `transfer`, `tree`, `upload`) plus `layouts/general.css` still carried literal px/rem `margin`/`padding`/`gap` values. The two skins that set `--space-factor` (brutalist 0.95, clay 1.06) and the `data-profile` density profiles therefore rescaled every other component while leaving these untouched — visibly off-rhythm spacing. All 151 values now follow the Phase 8.5 convention `calc(N * var(--space-factor, 1))` (tokenized via `tools/tokenize-spacing.mjs`; pixel-identical at the default factor), and `exsa.bundle.css` was regenerated. The token audit drops from 188 actionable values to 73, all of which are the generated `exsa.debug.css` dev overlay (never shipped, `data-debug` only).

- **Corner radii now follow skins and density profiles** — literal `border-radius` values in 17 components + 2 layouts ignored the skin shape tokens: micro radii (1–4px on calendar events, kbd chips, slider tracks, scrollbar thumbs, card badges, timeline rails, color-picker controls) now consume `var(--border-radius)`; the pill idiom (badges, tags, status pills, toggle tracks, upload progress bars, cart counts) now uses `calc(var(--border-radius) * 20)` — pixel-identical at defaults, follows every skin's radius (sharp under brutalist), scales with `--radius-factor`; transfer's check indicator uses `var(--surface-radius)` like its checkbox sibling. Kept deliberate: `50%` circles, the explicit `btn--radius-full`/`-none` utilities, and sharp resets (mobile topbar list, input-group inner field, hero figure, price superscript). Bundle regenerated.

- **Legacy degradation tiers — honest fallbacks, documented floor** — `color-mix()` fallback added to `exsa.overrides` (after the skins layer): 2022–early-2023 engines (`@layer` without `color-mix()`) now get flat, opaque surface tokens and static shadows, so floating surfaces and the topbar never render transparent and scrollbars keep a tint. `topbar.css`/`dashboard.css` gained `@supports not selector(:has(*))` guards pinning size variants to the default height in Firefox < 121, so body padding can't desync from the bar. Browser Support docs now spell out four tiers, including the hard `@layer` floor: pre-2022 browsers render unstyled, by design.
...
- **CI no longer depends on npm** — the `validate` workflow ran `npm test`, but `package.json` has no `test` script, so every push/PR failed with `Missing script: "test"` (and GitHub emailed on each one). The workflow now runs the real gate directly with Node — `node tools/validate.mjs --layers --no-legacy --check-tokens --check-bundles --check-debug` — matching the framework's zero-npm stance.

### 🐞 Fixed

- **Collapsed sidebar rail never collapsed inside layout shells** — `.sidebar--collapsed { width: 56px }` (layer `exsa.components`) always lost to the shells' width rules (later layer `exsa.layouts`): `general.css`'s `width: auto` on side pieces let the auto track size from the still-visible `.sidebar__head` content instead of the rail (139px on the Layouts page), the ≤860px stack's `width: 100%` ignored the collapse entirely, and the app frame's fixed `var(--layout-aside-start-w)` rail — plus the content pane's matching margin — stayed at full width. The collapsed width is now a token, `--sidebar-collapsed-w` (default 56px), owned by the component and read by both shells: `general.css` exempts collapsed pieces from its width overrides (desktop track and both ≤860px resets) so the rail keeps its icon width at any size and its auto track matches it exactly; `dashboard.css` re-points `--layout-aside-start-w` to the token when the rail is collapsed so the content pane's margin syncs (56px). The collapsed head now centers with zero inline padding, matching the fluid variant.

- **Topbar size variants never reached the body** — `body:has(.topbar--sm/lg/xl)` (0,1,1) always lost to the base `body.has-topbar:has(.topbar)` reservation (0,2,1): specificity beats source order, so on `topbar--xl` pages the bar rendered 72px while body padding and sticky offsets read 56px (a 16px mismatch). The variant sync now carries the `.has-topbar` prefix (0,3,1) and wins — caught by the paper-model migration, whose sticky-aside offset reads the same token.

- **Mobile topbar menu no longer shows every dropdown expanded** — the crash-safe open/close recipe in `head.php` pinned `.topbar__dd-menu` to `display: block` (opacity-toggled) so the desktop dropdown never flips display. Being unlayered, it beat the component's own rules everywhere — including mobile, where the menu is an in-flow accordion that relies on `display: none/block`: every dropdown rendered stacked between the links (the big gaps in the open mobile menu). The recipe is now scoped to `@media (min-width: 768px)`; ≤767px gets a recolor-only rule and the component owns show/hide.

- **Layouts responsive pass** — the three `dist/layouts/` shells got their missing mobile rules. `store.css` now stacks the filter/cart asides at ≤860px (full-width rows, sticky→static, 45vh cap), the topbar search shrinks instead of overflowing the bar, sticky-footer math accounts for the announce bar, sticky asides gained `--topbar-height` fallbacks, and the duplicated `.shop__card` family was removed — `components/product-card.css` is the single owner and store.css documents the link requirement. `general.css` moved its mobile block to a final ≤860px section so equal-specificity overrides win the cascade (this also fixed the topbar-content offset leaking onto mobile): `layout--aside-sticky` (now supports both asides) goes static on mobile, `blog--has-toc` stacks the TOC above the content, and the topbar/footer content toggles now follow left, right, or both asides and reset to full width on mobile. `dashboard.css`: the right panel stacks below the content at ≤767px with the shell taking over scrolling, the closed off-canvas sidebar is `visibility:hidden` (out of the tab order), the backdrop fades both ways and is mobile-scoped, and the dash tokens are scoped to shell pages instead of every `body`.
